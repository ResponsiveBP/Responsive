...
const RbpModal = (($d, core, base, w, d) => {
  ...
  const isExternalUrl = url => {
    ...
    if (
      !locationParts ||
      !locationParts[2] ||
      rlocalProtocol.test(locationParts[1])
    ) {
      return false;
    }
  ...
  function buildMain(complete) {
    ...
    const external = isExternalUrl(this.target);
    const iframeScroll = this.options.iframeScroll === true;
    const isImage = this.options.image === true || rimage.test(this.target);
    const isIframe =
      this.options.iframe === true || (notHash && external) ? !isImage : false;
    ...
    if (!this.local) {
      ...
      if (isIframe) {
        ...
        const src =
          external && this.target.indexOf("http") !== 0
  ...
  function destroyMain() {
    ...
    // TODO: Handle returning of local content.
    // https://github.com/ResponsiveBP/Responsive/blob/cef63b8367abae17dcf67ca0ff11a3f62ef90a1d/src/js/responsive.modal.js#L380
  }
  ...
  class RbpModal extends base {
    constructor(element, options) {
      ...
      this.overlay = fromTemplate(
        `<div role="document" class="modal-overlay fade-out"></div>`
      );
      this.dummy = $d.id("dmo");
      if (!this.dummy) {
        this.dummy = fromTemplate(
          `<div id="dmo" hidden class="fade-out"></div>`
        );
        $d.prepend(d.body, this.dummy);
      }

      this.modal = fromTemplate(
        `<div id="${modalId}" class="modal fade-out"></div>`
      );

      this.loader = fromTemplate(
        `<span class="modal-loader"><span class="vhidden">${
          this.options.loadHint
        }</span></span>`
      );

      this.closeTrigger = fromTemplate(
        `<button class="modal-close"><span class="vhidden">${
          this.closeHint
        }</span></button>`
      );

      this.prevTrigger = fromTemplate(
        `<button><span class="vhidden">${this.prevHint}</span></button>`
      );

      this.nextTrigger = fromTemplate(
        `<button class="forward"><span class="vhidden">${
          this.nextHint
        }</span></button>`
      );
      ...
      if (this.options.title) {
        ...
        this.header = fromTemplate(
          `<header><h2 id="${this.titleId}">${this.options.title}</h2></header>`
        );
      }
      ...
      if (this.options.description) {
        ...
        this.footer = fromTemplate(
          `<footer><p id="${this.descId}">${
      ...
      // A11y
      $d.setAttr([this.prevTrigger, this.nextTrigger], {
        tabindex: 0,
        "aria-controls": modalId
      });
      if (this.titleId || this.descId) {
        $d.setAttr(this.overlay, {
          "aria-labelledby": `${this.titleId || ""} ${this.descId || ""}`
        });
      }
    ...
    hideModal() {
      ...
      const complete = () => {
        ...
        // TODO: Reactivate when working on groups.
